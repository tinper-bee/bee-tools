...
// 读取进程参数值
function getArgsValue(argName){
	// console.log("getArgsValue--->"+argName,process.argv)
	//限制只能支持以下参数
	if(!['--JS_SRC','--JS_BUILD','--CSS_SRC','--CSS_BUILD'].includes(argName))return ;
	var argIdx = process.argv.indexOf(argName);
	if(argIdx<=0)return;
	var argVal = process.argv.slice(argIdx+1,argIdx+2);
	// console.log("getArgsValue--->value:",argVal[0]);
	return argVal[0];
}
...
gulp.task("pack_build", ["clean_build"], function (cb) {
	...
	var pkg = util.getPkg();
	var srcPath = getArgsValue('--JS_SRC');//读取外部参数，确定源码根路径
	var buildPath = getArgsValue('--JS_BUILD');//读取外部参数，确定编译后输出路径
	if(!srcPath)srcPath=path.join(process.cwd(),'/src');
...
gulp.task("sass_component", function () {
	var srcPath = getArgsValue('--CSS_SRC');//读取外部参数，确定源码根路径
	var buildPath = getArgsValue('--CSS_BUILD');//读取外部参数，确定编译后输出路径
	if(!srcPath)srcPath=path.join(process.cwd(),'/src');
	if(!buildPath)buildPath="build";
	gulp
		.src([path.join(srcPath, "/**/*.scss")])
		.pipe(sass())
		.pipe(gulp.dest(buildPath));
